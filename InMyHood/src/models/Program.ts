...
export function getProgramByCategory(category: string): Program[] {
  return ProgramSeed.filter(
    (program) => program.category.toLowerCase() === category.toLowerCase(),
  );
}

// The functions below are used to get programs by different criteria
export function getProgramsByDateRange(
  startDate: Date,
  endDate: Date,
): Program[] {
  return ProgramSeed.filter((program) => {
    ...
    return (
      (programStartDate >= startDate && programStartDate <= endDate) ||
      (programEndDate >= startDate && programEndDate <= endDate)
    );
  });
}
// The functions below are used to get programs by different criteria
export function getProgramsByLocation(location: string): Program[] {
  return ProgramSeed.filter(
    (program) => program.location.toLowerCase() === location.toLowerCase(),
  );
}
...
export function getProgramsByOrganizer(organizer: string): Program[] {
  return ProgramSeed.filter(
    (program) => program.organizer.toLowerCase() === organizer.toLowerCase(),
  );
}
